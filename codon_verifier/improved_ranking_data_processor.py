...
@dataclass
class ImprovedRankingDataConfig:
    """改进的排序数据配置"""
    # 数据过滤
    min_sequences_per_group: int = 2
    max_sequences_per_group: int = 100  # 增加最大组大小
    ...
    min_cluster_size: int = 5  # 最小聚类大小
    
    # 弱标签生成
    k_folds: int = 5
    weak_label_model: str = "random_forest"  # 使用随机森林替代LightGBM
...
class ImprovedRankingDataProcessor:
    """改进的排序数据处理器"""
    
    def __init__(self, config: ImprovedRankingDataConfig):
        self.config = config
    ...
    def cluster_proteins_by_similarity(self) -> Dict[str, int]:
        ...
        if self.config.n_clusters is None:
            # 基于数据量自动确定聚类数量
            n_clusters = max(2, min(20, n_groups // 10))
        else:
        ...
        clustering = AgglomerativeClustering(
            ...
            metric='precomputed',
            linkage='average'  # 使用average连接，与precomputed兼容
        )
    ...
    def create_pairwise_training_pairs(self, dataset_type: str) -> List[Tuple[int, int, int]]:
        """创建pairwise训练对"""
        ...
        logger.info(f"Created {len(pairs)} pairwise training pairs for {dataset_type}")
        return pairs
    
    def prepare_training_data(self, data_paths: List[str]) -> Dict[str, Any]:
        """完整的训练数据准备流程"""
        logger.info("Starting improved data preparation pipeline...")
        ...
        logger.info("Improved data preparation completed successfully!")
...
def create_improved_data_loaders(prepared_data: Dict[str, Any], 
                                batch_size: int = 32,
                                num_workers: int = 0) -> Tuple[DataLoader, DataLoader, DataLoader]:
    """创建改进的数据加载器"""
    
    # 创建数据集
    train_dataset = ImprovedRankingDataset(
    ...
    val_dataset = ImprovedRankingDataset(
        prepared_data["protein_groups"],
    ...
    test_dataset = ImprovedRankingDataset(
        prepared_data["protein_groups"],
