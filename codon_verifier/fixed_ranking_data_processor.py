...
解决关键问题：
1. 修复pair生成：确保训练对数量≥30,000
2. 修复评估指标：按组宏平均，而不是全局混合
3. 验证簇级切分：确保整簇分配到同一split
4. 增加统计稳健性：bootstrap CI，多次重复
"""
...
@dataclass
class FixedRankingDataConfig:
    """修复的排序数据配置"""
    # 数据过滤
    min_sequences_per_group: int = 2
    max_sequences_per_group: int = 200  # 增加最大组大小
    
    # 数据划分 - 重新平衡
    train_ratio: float = 0.6  # 60%
    val_ratio: float = 0.2    # 20%
    test_ratio: float = 0.2   # 20%
    
    # 聚类参数
    n_clusters: Optional[int] = None
    min_cluster_size: int = 10  # 增加最小聚类大小
    ...
    # Pair生成参数 - 关键修复
    min_train_pairs: int = 30000  # 最小训练对数量
    max_pairs_per_group: int = 200  # 每组最大pair数量
    use_weak_label_pairs: bool = True  # 使用弱标签生成额外pairs
    weak_label_ratio: float = 0.3  # 弱标签pair比例
...
class FixedRankingDataProcessor:
    """修复的排序数据处理器"""
    
    def __init__(self, config: FixedRankingDataConfig):
        self.config = config
    ...
    def cluster_proteins_by_similarity(self) -> Dict[str, int]:
        ...
        if self.config.n_clusters is None:
            # 基于数据量自动确定聚类数量
            n_clusters = max(2, min(30, n_groups // 20))  # 增加聚类数量
        else:
    ...
    def generate_weak_labels_improved(self) -> Dict[str, float]:
        """改进的弱标签生成"""
        logger.info("Generating weak labels using improved method...")
    ...
    def create_pairwise_training_pairs_fixed(self, dataset_type: str) -> List[Tuple[int, int, int]]:
        """修复的pairwise训练对创建 - 确保足够的训练对数量"""
        ...
        for group_key in group_keys:
            ...
            for i in range(len(sequences)):
                for j in range(i + 1, len(sequences)):
                    ...
                    # 创建标签：1表示seq_i > seq_j，0表示seq_i < seq_j
                    if expr_i > expr_j:
                        label = 1
                    elif expr_i < expr_j:
                        label = 0
        ...
        # 2. 如果训练对数量不够，使用弱标签生成额外pairs
        if dataset_type == "train" and len(pairs) < self.config.min_train_pairs and self.config.use_weak_label_pairs:
            logger.info(f"Training pairs ({len(pairs)}) below minimum ({self.config.min_train_pairs}), generating weak label pairs...")
    ...
    def prepare_training_data(self, data_paths: List[str]) -> Dict[str, Any]:
        """完整的训练数据准备流程"""
        logger.info("Starting fixed data preparation pipeline...")
        ...
        # 5. 生成弱标签
        weak_labels = self.generate_weak_labels_improved()
        
        # 6. 创建训练对 - 使用修复的方法
        train_pairs = self.create_pairwise_training_pairs_fixed("train")
        val_pairs = self.create_pairwise_training_pairs_fixed("val")
        test_pairs = self.create_pairwise_training_pairs_fixed("test")
        
        logger.info("Fixed data preparation completed successfully!")
...
class FixedRankingDataset(Dataset):
    """修复的排序数据集"""
...
def create_fixed_data_loaders(prepared_data: Dict[str, Any], 
                             batch_size: int = 32,
                             num_workers: int = 0) -> Tuple[DataLoader, DataLoader, DataLoader]:
    """创建修复的数据加载器"""
    
    # 创建数据集
    train_dataset = FixedRankingDataset(
    ...
    val_dataset = FixedRankingDataset(
        prepared_data["protein_groups"],
    ...
    test_dataset = FixedRankingDataset(
        prepared_data["protein_groups"],
